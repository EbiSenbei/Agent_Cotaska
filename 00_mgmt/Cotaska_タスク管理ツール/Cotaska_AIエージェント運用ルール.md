...
### 8.3.1 不具合対応タスクの完了保留ルール

不具合対応タスクは、AIが修正・ビルド確認を終えてもユーザーの動作確認が完了するまでは `status: done` にしない。
修正後はユーザー確認待ちとして次を揃える。

1. `status: todo`
2. `progress_status: 仕掛`
3. `completed_at: null`
4. `updated_at` を更新
5. `due_date` が未設定でも、完了扱いではないため補完しない

ユーザーが動作確認後に完了を指示した場合のみ、8.3 の完了時ルールに従って `status: done` とする。
このとき `due_date` が未設定なら、今日のローカル日付（`YYYY-MM-DD`）を設定する。
